...
public class UpgradeBookServiceTest {
    ...
    @Test
    @DisplayName("빈도 기반 캐싱 적용 후 성능 측정 - 인기 키워드 시나리오")
    void upgradeAuthorSearchTest() {
        SearchFilterDto filterDto = new SearchFilterDto();
        filterDto.setBookAuthor("78");
        int threshold = 5; // 설정한 임계치
        PageRequest pageRequest = PageRequest.of(0, 10);
    ...
    @Test
    @DisplayName("고도화 로직 성능 - 딥 페이징 (1000페이지)")
    void upgreadeDeepPagingSearch(){
        ...
        // 1000번째 페이지 조회
        Page<BookResDto> result = bookService.findBooksWithFilterAndFTS(filter, PageRequest.of(1000, 10));
